...
Public Class ClassCustomerDB
    ...
    Public Sub GetCitiesSP(strName As String)
        ' CREATES INSTANCES OF THE CONNECTION AND COMMAND OBJECT
        Dim objConnection As New SqlConnection(mstrConnection)
        ' Tell SQL server the name of the stored procedure you will be executing
        Dim mdbDataAdapter As New SqlDataAdapter(strName, objConnection)
        Try
            ...
            Me.mDatasetCustomer.Clear()
            ' OPEN CONNECTION AND FILL DATASET
            mdbDataAdapter.Fill(mDatasetCustomer, "tblCities") 'change to sub-table you want to create!
    ...
    Public Sub GetCities()
        ...
        'Author: Louis Andres
        'Date: 3/22/15
        GetCitiesSP("usp_customers_get_cities") 'calling cities sub to call SP
    End Sub

    Public Sub SearchByCity2(strIn As String)
        'Purpose: search by username
        'Arguments: username (partial)
        'Returns: customers with certain username
        'Author: Louis Andres
        'Date: 3/22/15
        mMyView.RowFilter = "city = '" & strIn & "'"
    End Sub
